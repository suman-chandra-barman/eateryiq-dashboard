  ...
  return (
    <>
      ...
      >
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.href}>
                <Link
                  href={item.href}
                  onClick={() => item.subItems && toggleSubItems(item.href)}
                >
                  <Button
                    variant="ghost"
                    className={cn(
                    ...
                    {item.subItems && !isCollapsed && (
                      <ChevronRight
                        className={cn(
                          ...
                          showSubItems[item.href] && "rotate-90"
                        )}
                      />
                    )}
                  </Button>
                </Link>
                {!isCollapsed && item.subItems && showSubItems[item.href] && (
        ...
        {/* ===== User Profile ===== */}
        <div className="bg-white p-4 text-[#3B3B3B] space-y-4 border-t rounded-b-2xl">
          <button
